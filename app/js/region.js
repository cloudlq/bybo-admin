define(['moment',  'text!../tpl/region_opt.html', 'daterangepicker', 'bootstrap_table_export', 'fun', 'ajaxfileupload'], function(moment, _opt) {
	...
	var controller = function() {
		...
		IMPL.FIND = function(id, callback) {
			$.ajax({
				url: 'admin/get'+MODEL+'ById?regionId=' + id,
				type: "GET",
				contentType: "application/json",
				dataType: "json",
				success: callback
		...
		IMPL.DEL = function(id, callback) {
			$.ajax({
				url: 'admin/delete'+MODEL+'ById?regionId=' + id,
				type: "GET",
				contentType: "application/json",
				dataType: "json",
				success: callback
...
//				height: 500, //行高，如果没有设置height属性，表格自动根据记录条数觉得表格高度
				...
				columns: [{
					...
					formatter: function(value, row) {
						var html = '';
						html += '<span class="opt_btn invest_edit" data-id="' + row.regionId + '">修改</span><span class="opt_btn invest_delete" data-id="' + row.regionId + '">删除</span>';
						return html;
					},
					valign: 'middle'
				}],
		...
		EVENT.INIT = function() {
			...
			$("#table").on("click", '.invest_edit', function() {
				...
				IMPL.FIND(id, function(resp) {
					...
					$("#cnName").val(resp.cnName);
					$("#enName").val(resp.enName);
					$("#cnAddress").val(resp.cnAddress);
					$("#enAddress").val(resp.enAddress);
					$("#email").val(resp.email);
					$("#url").val(resp.url);
					$("#imgPath").attr("src", resp.logoImageUrl);
			...
			$(".boxSubmit").click(function() {
				...
				var cnAddress = $("#cnAddress").val();
				var enAddress = $("#enAddress").val();
				var email = $("#email").val();
				var url = $("#url").val();
				var phone = $("#phone").val();
				var logoImageUrl = $("#imgPath").attr("src");
				
				if(_FUN.isNull(cnName)) {

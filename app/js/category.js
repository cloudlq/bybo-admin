define(['moment',  'text!../tpl/category_opt.html', 'daterangepicker', 'bootstrap_table_export', 'fun', 'ajaxfileupload'], function(moment, _opt) {
...
//				height: 500, //行高，如果没有设置height属性，表格自动根据记录条数觉得表格高度
				...
				columns: [{
					...
					width:"25%",
					formatter: function(value, row) {
						var html = '<span class="opt_btn invest_edit" data-id="' + row.id + '">修改</span><span class="opt_btn invest_delete" data-id="' + row.id + '">删除</span>';
						return html;
					},
					valign: 'middle'
				}],
				searchHtml: '<div style="float:left;width:100%"><div class="pull-right search"><select class="form-control"  id="language-search"><option value="">语言</option><option value="cn">中文</option><option value="en">英文</option></select></div><div class="pull-right search"> <select class="form-control" id="isshow-search"><option value="">是否首页</option><option value="1">显示</option><option value="0">不显示</option></select></div></div>'
		...
		EVENT.INIT = function() {
			...
			$("#table").on("click", '.invest_edit', function() {
				...
				IMPL.FIND(id, function(resp) {
					...
					$("input[name=isShow][value="+isShow+"]").attr("checked",true);
					
					FUN.REND_DEPART_EDIT(resp.departmentId+'%&%'+resp.department);
				});
			});
			...
			$(".boxSubmit").click(function() {
				...
				var departmentVal = $("#departmentList").val();
				var departmentId = departmentVal.split("%&%")[0];
				var department= departmentVal.split("%&%")[1];
